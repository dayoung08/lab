...
void algorithm_run(server* _server_list, channel* _channel_list, bitrate_version_set* _version_set, double _cost_limit, int _model) {
	...
	if (_model == CPU_USAGE_MODEL) {
		TA_usage_phase(_server_list, _channel_list, _version_set, _cost_limit, selected_set, selected_ES, _used_GHz, _ES_count, _model);
	}
	if (_model == LEASING_MODEL) {
		TA_leasing_phase(_server_list, _channel_list, _version_set, _cost_limit, selected_set, selected_ES, _used_GHz, _ES_count, _model);
	}
	total_GHz = 0;
	total_pwq = 0;
	for (int ch = 1; ch <= CHANNEL_NUM; ch++) {
...
void TA_usage_phase(server* _server_list, channel* _channel_list, bitrate_version_set* _version_set, double _cost_limit, short* _selected_set, short** _selected_ES, double* _used_GHz, short* _ES_count, int _model) {
	// 2-1. TA phase
...
void TA_leasing_phase(server* _server_list, channel* _channel_list, bitrate_version_set* _version_set, double _cost_limit, short* _selected_set, short** _selected_ES, double* _used_GHz, short* _ES_count, int _model) {
	// 2-1. TA phase
	set<pair<double, int>> remained_GHz_of_ESs_set;
	for (int ES = 1; ES <= ES_NUM; ES++) {
		remained_GHz_of_ESs_set.insert(make_pair(_server_list[ES].processing_capacity, ES)); //set
...
void CR_leasing_phase(server* _server_list, channel* _channel_list, bitrate_version_set* _version_set, double _total_cost, double _cost_limit, short* _selected_set, short** _selected_ES, double* _used_GHz, short* _ES_count, int _model) {
	if (_total_cost > _cost_limit) {
		...
		while (list_CR.size() && flag) {
			...
			double prev_cost = calculate_ES_cost(&(_server_list[ES]), _used_GHz[ES], _model);
			_ES_count[ES] = 0;
			_used_GHz[ES] = 0;
			_total_cost -= prev_cost;
			...
			while (!pwq_of_version_in_ES.empty()){
				...
				int ver_in_ES = (*pwq_of_version_in_CTS.begin()).second.second;
