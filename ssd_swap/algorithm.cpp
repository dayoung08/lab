...
void run(SSD* _SSD_list, video_segment* _segment_list, int _method) {
	switch (_method) {
	case 1:
		our_algorithm(_SSD_list, _segment_list);
		break;
	case 2:
		//benchmark(_SSD_list, _segment_list);
		break;
	}
}


void our_algorithm(SSD* _SSD_list, video_segment* _segment_list) {
...
pair<double, double> get_slope_to(SSD* _SSD_list, video_segment* _segment_list, int _from_ssd, int _to_ssd, int _from_seg, bool* _is_full) {
	...
	slope_to = (_SSD_list[_to_ssd].maximum_bandwidth - _SSD_list[_to_ssd].bandwidth_usage - bt_difference) * ADWD_to;
...
pair<double, double> get_slope_from(SSD* _SSD_list, video_segment* _segment_list, int _from_ssd, int _to_ssd, int _from_seg, bool* _is_full) {
	...
	slope_from = (_SSD_list[_from_ssd].maximum_bandwidth - _SSD_list[_from_ssd].bandwidth_usage + bt_difference) * ADWD_from;
