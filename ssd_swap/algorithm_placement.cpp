	...
	while (!VIDEO_SEGMENT_list_with_bandwidth_sort.empty()) {
		...
		for (int ssd_temp = 0; ssd_temp < NUM_OF_SSDs; ssd_temp++) {
			if (!is_not_enough_storage_space(_SSD_list, _VIDEO_SEGMENT_list, ssd_temp, video_index) &&
				...
				//낮을수록 좋아야함 분명히....
				double bb = (_SSD_list[ssd_temp].maximum_bandwidth - (_SSD_list[ssd_temp].bandwidth_usage + _VIDEO_SEGMENT_list[video_index].requested_bandwidth));
				//double ss = (_SSD_list[ssd_temp].storage_capacity - (_SSD_list[ssd_temp].storage_usage + _VIDEO_SEGMENT_list[video_index].size)); 
				//bb / ss =남은 밴드윗/공간
				
				double ll = (_SSD_list[ssd_temp].storage_capacity * _SSD_list[ssd_temp].DWPD) - (_SSD_list[ssd_temp].write_MB + _VIDEO_SEGMENT_list[video_index].size);
...
/*
int placement_random(SSD* _SSD_list, VIDEO* _VIDEO_SEGMENT_list) { // 밴드윗 생각 전혀 없음
	int placement_num = 0;
	std::mt19937 g(SEED + rand_cnt_for_placement);
	rand_cnt_for_placement++;
	vector<int> target_ssd_list;
